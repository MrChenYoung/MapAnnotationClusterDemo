...
QuadTreeNodeData QuadTreeNodeDataForAMapPOI(CLLocation* poi)
{
...
{
    CLLocationDegrees minX = ((CLLocation *)pois[0]).coordinate.latitude;
    CLLocationDegrees maxX = ((CLLocation *)pois[0]).coordinate.latitude;
    
    CLLocationDegrees minY = ((CLLocation *)pois[0]).coordinate.longitude;
    CLLocationDegrees maxY = ((CLLocation *)pois[0]).coordinate.longitude;
            ...
            QuadTreeGatherDataInRange(self.root, BoundingBoxForMapRect(mapRect), ^(QuadTreeNodeData data)
                                      {
                                          ...
                                          [pois addObject:(__bridge CLLocation *)data.data];
                                      });
...
- (void)buildTreeWithLocations:(NSArray <CLLocation *>*)locations
{
    QuadTreeNodeData *dataArray = malloc(sizeof(QuadTreeNodeData) * [locations count]);
    
    BoundingBox maxBounding = quadTreeNodeDataArrayForPOIs(dataArray, locations);
    ...
    NSLog(@"build tree.");
    /*建立四叉树索引. */
    self.root = QuadTreeBuildWithData(dataArray, [locations count], maxBounding, 4);
